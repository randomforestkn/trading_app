...
class ActivityScreen extends StatelessWidget {
                    ...
                    Padding(
                      ...
                      child: Align(
                        ...
                        child: AppSecondaryButton(
                          ...
                          onPressed: () => _openJournalNote(context, order),
                        ),
                      ),
                    ),
                  ],
  ...
  void _openJournalNote(BuildContext context, PaperOrder order) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => JournalEditorScreen(
          prefillTitle: '${order.assetSymbol} ${order.side.label} review',
          prefillBody:
              'Executed ${order.quantity.toStringAsFixed(4)} shares at \$${order.executionPrice.toStringAsFixed(2)}.\n'
              'Estimated total: \$${order.estimatedTotal.toStringAsFixed(2)}.',
          prefillLinkedOrderId:
              '${order.assetSymbol}-${order.side.name}-${order.timestamp.microsecondsSinceEpoch}',
          prefillLinkedAssetSymbol: order.assetSymbol,
          prefillLinkedStrategy: JournalStrategyType.stockTrade,
          prefillMood: JournalMood.disciplined,
          prefillOutcome: JournalOutcome.open,
          prefillTags: [order.assetSymbol, order.side.label.toLowerCase()],
        ),
      ),
    );
  }
